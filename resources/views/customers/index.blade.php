                                ...
                                <div class="client__thumb mb-15">
                                    <a href="{{ route('customers.show', $customer->id) }}">
                                        <img src="{{ $customer->image ? asset('storage/' . $customer->image) : asset('img/user.png') }}"
                                            alt="{{ $customer->name }}" class="img-fluid rounded-circle"
                                ...
                                <div class="client__content">
                                    <div class="client__meta">
                                        ...
                                        @if ($customer->assignedAgent)
                                            <p><i class="fas fa-user-tie me-1"></i> Assigned to:
                                                {{ $customer->assignedAgent->name }}</p>
                                        @else
                                            <p><i class="fas fa-exclamation-triangle me-1 text-warning"></i> No Agent
                                                Assigned</p>
                                        @endif
                                    </div>
                                    <div class="common-social mb-20">
                                        @if ($customer->getMeta('facebook'))
                                            <a href="{{ $customer->getMeta('facebook') }}" target="_blank"><i
                                                    class="fa-brands fa-facebook-f"></i></a>
                                        @endif
                                        @if ($customer->getMeta('twitter'))
                                            <a href="{{ $customer->getMeta('twitter') }}" target="_blank"><i
                                                    class="fa-brands fa-x-twitter"></i></a>
                                        @endif
                                        @if ($customer->getMeta('linkedin'))
                                            <a href="{{ $customer->getMeta('linkedin') }}" target="_blank"><i
                                                    class="fa-brands fa-linkedin-in"></i></a>
                                        @endif
                                        @if ($customer->getMeta('youtube'))
                                            <a href="{{ $customer->getMeta('youtube') }}" target="_blank"><i
                                                    class="fa-brands fa-youtube"></i></a>
                                        @endif
                                        @if ($customer->getMeta('website'))
                                            <a href="{{ $customer->getMeta('website') }}" target="_blank"><i
                                                    class="fa-thin fa-globe"></i></a>
                                        @endif
                                    </div>
